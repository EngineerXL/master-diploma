...
Разработать и исследовать алгоритм лидарной одометрии беспилотного автомобиля, устойчивый к частичному перекрытию поля зрения крупным динамическим объектом. В работе необходимо провести анализ существующих подходов к регистрации облаков точек, формализовать сценарий перекрытия поля зрения, реализовать алгоритм обработки последовательности лидарных кадров с использованием вокселизации, взвешенного ICP, модели движения и фильтра Калмана, а также выполнить экспериментальную оценку качества по компонентам линейной и угловой скорости. Исходные данные: лидарные последовательности и эталонные оценки движения из набора данных Boreas-RT; модельный сценарий обгона грузовиком; программная среда на языке Python для воспроизводимого запуска вычислительных экспериментов.
...
\fullcite{scipy-ckdtree}\\
\fullcite{scipy-rotation}
